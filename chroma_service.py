...
class ChromaService:
    ...
    def add_chunks(self, chunks: List[str], meta_data: dict = {}):
        ...
        for _, chunk in enumerate(chunks):
            ...
            documents.append(
                Document(page_content=chunk, meta_data=meta_data)
            )
    ...
    def query(self, query_text: str, 
              meta_data: dict[str, str]= {}) -> List[Document]:

        docs = self.vector_store.similarity_search(query=query_text,
                                                   filter=meta_data)
        return docs
